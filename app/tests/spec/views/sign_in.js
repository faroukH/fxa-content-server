      ...
      it('does not show if there is an email in relier that does not match', function () {
        ...
        return view.render()
          .then(function () {
            ...
            assert.ok(view.$('.password').length, 'should show password input');
            assert.isTrue(TestHelpers.isEventLogged(metrics, 'signin.ask-password.shown.no-account'));
          });
      });
      ...
      it('asks for password right away if service is sync', function () {
        ...
        return view.render()
          .then(function () {
            ...
            assert.isTrue(TestHelpers.isEventLogged(metrics, 'signin.ask-password.shown.wants-keys'));
          });
      });
      ...
      it('asks for the password if the stored session is not from sync', function () {
        ...
        return view.render()
          .then(function () {
            assert.equal($('.email')[0].type, 'hidden', 'should not show email input');
            assert.ok($('.password').length, 'should show password input');
            assert.isTrue(TestHelpers.isEventLogged(metrics, 'signin.ask-password.shown.non-sync-account'));
          });
      });
      ...
      it('asks for the password when re-rendered due to an expired session', function () {
        ...
        return view.render()
          .then(function () {
            assert.equal($('.email')[0].type, 'hidden', 'should not show email input');
            assert.ok($('.password').length, 'should show password input');
            assert.isTrue(TestHelpers.isEventLogged(metrics, 'signin.ask-password.shown.expired-session'));
          });
      });
    });
